...
if __name__ == "__main__":
    ...
    #Also do with most popular for extra 5
    ...
    transactionScams = transactionScams.filter(filterByLucrative)
    transactionScams = transactionScams.map(lambda x: (x[1][0][0], x[1][0][1]))
    valuesRecords=spark.sparkContext.broadcast(transactionScams.countByKey())
    transactionScams = transactionScams.reduceByKey(lambda x,y: x+y)
    transactionScams = transactionScams.map(lambda x:(x[0],x[1]/valuesRecords.value[x[0]]))
    
    print(transactionScams.take(100))
    ...
    my_result_object = my_bucket_resource.Object(s3_bucket,'scamstimefixed' + date_time + '/partd_scamstimefixed.txt')
    my_result_object.put(Body=json.dumps(transactionScams.take(10000)))
